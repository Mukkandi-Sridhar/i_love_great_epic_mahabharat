...
const Payment = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-primary/30 font-sans pb-20">
      <header className="px-5 md:px-8 h-16 flex items-center justify-between border-b border-white/5 bg-black/40 sticky top-0 z-50 backdrop-blur-md">
      ...
      {/* Single column on mobile; on desktop the form and the order summary sit
          side by side so the total and pay button stay visible while scrolling. */}
      <div className="mx-auto w-full max-w-[480px] lg:max-w-5xl px-5 md:px-8 py-8 lg:grid lg:grid-cols-[minmax(0,1fr)_380px] lg:gap-12 lg:items-start">
        <div className="space-y-10 lg:space-y-8">
          <div className="flex items-center gap-5 p-4 bg-white/[0.02] rounded-3xl border border-white/5 lg:hidden">
        ...
        <aside className="pt-10 lg:pt-0 space-y-6 lg:sticky lg:top-24">
          <div className="hidden lg:flex items-center gap-4 p-4 bg-white/[0.02] rounded-3xl border border-white/5">
            <div className="w-16 h-16 bg-black/40 rounded-2xl p-2 border border-white/5 flex items-center justify-center shrink-0">
              <img src={product.image} alt={product.title} className="w-full h-full object-contain" />
            </div>
            <div className="flex-1 min-w-0">
              <h2 className="text-sm font-bold truncate uppercase tracking-tight">{product.title}</h2>
              <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">
                {product.type === "ebook" ? "Digital Access" : "Physical Artifact"}
              </p>
            </div>
          ...
          <div className="lg:rounded-3xl lg:border lg:border-white/5 lg:bg-white/[0.02] lg:p-6 space-y-6">
            ...
            <div className="flex items-end justify-between px-2 lg:px-0">
              <div>
                ...
                <div className="flex items-baseline gap-2 flex-wrap">
                  {finalPrice <= 0 && basePrice > 0 ? (
                    <>
                      <span className="text-4xl font-black text-emerald-400 px-1">FREE</span>
                      <span className="text-sm text-gray-500 line-through">Rs.{basePrice}</span>
                    </>
                  ) : (
